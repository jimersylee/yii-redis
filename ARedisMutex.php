...
class ARedisMutex extends ARedisEntity {
    ...
    public function lock() {
        ...
        if (!$redis->setnx($this->name, $this->getExpiresAt(true))) {
            ...
            if ($value > $this->getMillisecond()) {
                
            
                return false;
                
            } else {
                //prevent a race condition if another process finds it expired at the same time
    ...
    public function unlock() {
        ...
        if (bccomp($value, $this->_expiresAt, $decimalPlaces) == -1 && bccomp($value, $this->getMillisecond(), $decimalPlaces) == 1) {
            return false;
        }
    ...
    public function getExpiresAt($forceRecalculate = false) {
        if ($forceRecalculate || $this->_expiresAt === null) {
            $this->_expiresAt = $this->expiresAfter * 1000 + $this->getMillisecond();
        }
